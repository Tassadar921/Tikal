...
export class CheckingService {
  ...
  checkPassword = (password, conf=false) => {
    if(!conf){
      this.outputPassword = Object(this.translationService.dictionary)
        .connection[this.checkPasswordIntermediary(password)]
        .data;
    }else{
      this.outputConfPassword = Object(this.translationService.dictionary)
        .connection[this.checkPasswordIntermediary(password)]
        .data;
    }
    this.checkPasswords();
  ...
  checkPasswords = () => {
    ...
    if(!this.arePasswordsOK){
      if(!this.outputPassword && !this.outputConfPassword){
        this.outputPassword = Object(this.translationService.dictionary).connection[8].data;
        this.outputConfPassword = Object(this.translationService.dictionary).connection[8].data;
      }
    }else{
      this.outputPassword = '';
  ...
  updateEmail = (email) => Object(this.translationService.dictionary)
    .connection[this.checkEmail(email)]
    .data;

  //triggers on keyup in username input of signUp
  updateUsername = (username) => Object(this.translationService.dictionary)
